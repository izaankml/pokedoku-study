...
export function intervalFor(streak) {
  const s = Math.max(0, streak | 0);
  return INTERVALS_MS[Math.min(s, INTERVALS_MS.length - 1)];
}

// Interval that would apply after answering `correct` from state `entry`.
export function nextInterval(entry, correct) {
...
export function dueFactor(entry, now) {
...
export function scheduleStatus(entry, now) {
...
export function scheduleSummary(table, keys, now) {
  const out = { new: 0, due: 0, learning: 0, mastered: 0 };
