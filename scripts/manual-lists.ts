...
import type { EvoMethod, Region } from "../src/data/types.ts";

// An entry PokeDoku lists on its own with no dex forme behind it: a clone
// of the species record under PokeDoku's id (see CLONED_FORMS).
export interface ClonedForm {
  // PokeDoku's id for it (90000+ for its own cosmetic entries)
  id: number;
  // the species' dex number
  species: number;
  form: string;
  // the dex-style slug to give it
  name: string;
  // the record it evolved from, when it keeps the line's form
  prevo?: number;
}
...
export const HISUI_FORM_IDS = new Set<string>([
...
export const EVO_ITEM_OVERRIDES: Record<string, string> = {
...
export const EVO_DETAIL_OVERRIDES: Record<string, string> = {
...
export const FORM_DEBUT_REGION_OVERRIDES: Record<string, Region> = {
  deoxysattack: "kanto",
  deoxysdefense: "kanto",
  hoopaunbound: "hoenn",
};
